...
@pytest.mark.parametrize(
    ...
    (
        (
            {"username": "test_author", "password": "123654", "email": "author@example.com"},
            {"title": "Test title", "content": "Some test content"},
            status.HTTP_201_CREATED
        ),
    ),
)
async def test_create_blog(async_client: AsyncClient, user_payload, blog_payload, status_code):
    """
...
@pytest.mark.parametrize(
    ...
    (
        (
            {"username": "test_author", "password": "123654", "email": "author@example.com"},
            {"author_id": 1, "title": "Blog title", "slug": "blog-title",
             "content": "Some test content", "is_active": True},
            status.HTTP_200_OK
        ),
    ),
)
async def test_get_list_blog(async_client: AsyncClient, blog_payload, user_payload, status_code):
    """
